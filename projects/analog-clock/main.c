...
int app_main(int argc, char **argv) {
  ...
  const int show_date = f->data[1][0] != '0';
  const int minimal = f->data[1][2] == 'm';
  const int cx = f->width / 2, cy = show_date ? f->height / 2 - 5 : f->height / 2;
  const int r = (f->width < f->height - (show_date ? 14 : 0) ? f->width : f->height - (show_date ? 14 : 0)) / 2 - 7;
  f->ring(f->canvas, cx, cy, r, 1, f->muted);
  for (int i = 0; !minimal && i < 12; ++i) {
  ...
  if (show_date) {
    char date[16];
    date[0] = '0' + f->day / 10; date[1] = '0' + f->day % 10; date[2] = '/';
    date[3] = '0' + f->month / 10; date[4] = '0' + f->month % 10; date[5] = 0;
    f->label(f->canvas, f->height - 10, date, f->muted, 1);
  }
  return 0;
}
