...
Screen (`src/display/lcd_display.*`): top bar with Wi-Fi (signal bars) and Bluetooth icons, clock, status or notification text, mute and battery icons, low battery popup, light/dark theme. Battery firmware v1.1.10 takes 15 calibrated ADC samples once per second, removes the highest/lowest 20%, applies an exponential filter, and maps the filtered terminal voltage through a continuous Li-ion discharge curve. The console stores both estimated percent and millivolts in its one-minute telemetry history and charts them separately; this remains a voltage-based estimate rather than coulomb counting.
...
Projects are **independent native Xtensa ELF binaries**, uploaded into the board’s flash filesystem. Their implementations live under `projects/`, outside the base firmware. The base firmware contains the generic Espressif ELF loader and the versioned display API (`src/runtime/project_api.h`). Project binaries are never linked into the firmware.
...
- **Internet radio:** live MP3 stations over Wi-Fi on the ESP32-S3-LCD-0.85 speaker, with 13 built-in stations and up to three custom streams. Radio v1.7.0 requires base firmware v1.1.26 (project ABI 7). Select a starting station in Projects. On the board, double-tap Power to open or close the station list, use + or − to browse, and tap Power to play the selected station. Outside the list, tap + or − to adjust volume, hold to ramp volume, or double-tap + or − to change stations. Press + and − together for the system menu. The firmware decodes audio in a background task and stops playback when the project is unloaded. Station URLs are direct MP3 streams and may change over time.
...
The **Project Builder** tab provides an Arduino-style `.ino` editor without linking a second Arduino core into the display module. A sketch includes only `project_builder.h`, implements `setup(ProjectBoard *)` and `loop(ProjectBoard *)`, and ends with `PROJECT_SKETCH()`. The server compiles it into the same validated, self-describing Xtensa ELF used by the Projects tab. Drafts save automatically in browser storage; source and ELF files can be downloaded, and a successful ELF can be installed directly on a paired ESP32-S3-LCD-0.85 board. Editing a sketch during compilation requires another build before installation.

`project_builder.h` is the stable, single-file entry point for project code. ABI 4 exposes clipped drawing, time, buttons, battery, LED control, sandboxed SD read/write, bounded stereo microphone/speaker buffers, asynchronous Wi-Fi scan and HTTP(S) GET, and asynchronous BLE scan. The English guide and examples open inside the Project Builder tab. Projects never receive raw GPIO, credentials, firmware globals or another project's files.
...
**Over USB (base firmware v1.1.9):** when the board is plugged into the computer, the file manager uses it directly through Web Serial instead of the console relay. A port this site was already granted connects silently when the file manager opens (the board's MAC must match the device); otherwise **Connect over USB** opens the browser's port chooser. Listing, new folder, rename and delete answer in well under 100 ms, uploads run at about 250 KB/s and downloads at about 650 KB/s, and files up to 512 MB are allowed. Format in Configure also goes over USB when a granted port is plugged in. The port is released when the file manager closes, so the Flash firmware tab can use it again. Serial protocol (`src/services/serial_fs.*`): `fs ls|get|put|rm|mkdir|mv|mount|unmount|format|info` with percent-encoded paths; replies are JSON lines starting `{"fs"`. Uploads send raw blocks that fit the 4 KB serial receive queue, each acknowledged, and are checked with SHA-256; downloads arrive as base64 lines. Without USB, operations report measured progress (bytes, or sectors while formatting) on each check-in, and the console answers a board with a running SD operation immediately instead of holding its check-in, so results arrive within about a second.
