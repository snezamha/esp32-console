...
export function inspectProject(bytes: Uint8Array): ProjectMetadata {
  ...
  if (!metadata || !/^[a-z0-9][a-z0-9-]{0,47}$/.test(metadata.id) || metadata.id === "none" || typeof metadata.name !== "string" || !metadata.name.length || metadata.name.length > 80 || !/^\d+\.\d+\.\d+$/.test(metadata.version) || metadata.board !== "esp32-s3-lcd-0.85" || metadata.abi !== 1) return fail();
