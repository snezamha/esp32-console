...
export function inspectProject(bytes: Uint8Array): ProjectMetadata {
  const fail = (): never => { throw new Error("Invalid project file. Choose a standalone ESP32 project .elf built for ABI 2, 3, 4, 5, 6 or 7."); };
  ...
  if (!metadata || !/^[a-z0-9][a-z0-9-]{0,47}$/.test(metadata.id) || metadata.id === "none" || typeof metadata.name !== "string" || !metadata.name.length || metadata.name.length > 80 || !/^\d+\.\d+\.\d+$/.test(metadata.version) || metadata.board !== "esp32-s3-lcd-0.85" || ![2, 3, 4, 5, 6, 7].includes(metadata.abi)) return fail();
