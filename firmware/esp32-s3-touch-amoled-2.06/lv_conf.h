...
#define LV_STDARG_INCLUDE       <stdarg.h>
    ...
    #if LV_MEM_ADR == 0
        /* Take the pool from PSRAM instead of letting LVGL declare it as a
         * static array. That array lands in internal RAM, and on an ESP32-S3
         * LV_MEM_SIZE of it is internal RAM no one else can have: mbedtls is
         * built with CONFIG_MBEDTLS_INTERNAL_MEM_ALLOC, so a TLS handshake
         * needs ~35 kB of internal heap in one piece and fails with
         * "SSL - Memory allocation failed" when LVGL has taken it first. */
