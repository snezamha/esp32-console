...
void ConsoleClient::HandlePoll(int http_code, const std::string& body) {
  ...
  if (status == "pending") {
    ...
    if (token != token_) {
      ...
      Save();
      Board::GetInstance().ShowNotification("Device linked");
    ...
    if (!token_.empty()) {
      ...
      name_.clear();
      Save();
      Board::GetInstance().ShowNotification("Device removed");
...
void ConsoleClient::FinishOta(uint32_t now_ms) {
  ...
  if (ota_ok_) {
    ...
    Board::GetInstance().ShowNotification("Update installed", 5000);
    ...
    Board::GetInstance().ShowNotification("Update failed", 5000);
  }
